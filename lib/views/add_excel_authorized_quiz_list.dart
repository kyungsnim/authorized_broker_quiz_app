...
class AddExcelAuthorizedQuizList extends StatefulWidget {
  @override
  _AddExcelAuthorizedQuizListState createState() => _AddExcelAuthorizedQuizListState();
}

class _AddExcelAuthorizedQuizListState extends State<AddExcelAuthorizedQuizList> {
  ...
  uploadAuthorizedQuizData() async {
    try {
      setState(() {
        _isLoading = true;
      });
      ...
      await databaseService
          .addAuthorizedQuestionData(questionMap, quizId) //*********************
...
//    var excel = Excel.decodeBytes(bytes, update: true);
    ...
    for (var table in excel.tables.keys) {
      ...
      for (var row in excel.tables[table].rows) {
        ...
        uploadAuthorizedQuizData();
      }
    }
  }

  importAuthorizedExcelData() async {

    ByteData data = await rootBundle.load("assets/authorized_broker_quiz.xlsx");
    List<int> bytes = data.buffer.asUint8List(data.offsetInBytes, data.lengthInBytes);
//    var excel = Excel.decodeBytes(bytes, update: true);
    var excel = Excel.decodeBytes(bytes);
    ...
    for (var table in excel.tables.keys) {
      for (var row in excel.tables[table].rows) {
        ...
        option3 = data[6].toString();
        option4 = data[7].toString();
        option5 = data[8].toString();
        hint = data[9].toString();
        hintImg = data[10].toString();
        whenDid = data[11].toString();
        fromWho = data[12].toString();
        createDt = data[13].toString();
        updateDt = data[14].toString();
...
//                      importExcelData();
                  ...
                  InkWell(
                    onTap: () {
                      // 여기서 엑셀파일 열어서 데이터들 가져와서 for문 돌리자
                      importAuthorizedExcelData();
                    },
                    child: blueButton(
                      context: context,
                      text: 'Add Authorized Questions',
                      buttonWidth: MediaQuery.of(context).size.width * 0.44,
                    ),
                  ),
                ],
